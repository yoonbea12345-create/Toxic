...
async function callWithFallback(params: Omit<Anthropic.MessageCreateParamsNonStreaming, 'model'>) {
  try {
    return await callModel(params, MODEL_SONNET, 40000);
  } catch {
    return await callModel(params, MODEL_HAIKU, 15000);
  }
}
...
function buildPhase1Prompt(myData: any, targetData: any, relationType: string, result: any): string {
  ...
  if (hasTarget) {
    return `두 사람의 사주로 01~03 섹션 분석을 해주세요.
...
[충돌] 충: ${result.conflicts.chung.map((c: any) => c.name).join(', ') || '없음'} / 형: ${result.conflicts.hyung.map((h: any) => h.name).join(', ') || '없음'} / 해: ${result.conflicts.hae.map((h: any) => h.name).join(', ') || '없음'} / 오행극: ${result.conflicts.geuk.direction || '없음'}

순수 JSON으로만 반환 (코드블록 없이):

{
  "toxicSummary": "15자 이내 핵심 한 줄",
  "coreConflict": { "title": "갈등 구조 이름 (10자 이내)", "description": "근본 충돌 이유 2문장. 단정적으로" },
  "conflictAnalysis": { "chung": "충 있을 때 1문장 (없으면 null)", "hyung": "형 있을 때 1문장 (없으면 null)", "hae": "해 있을 때 1문장 (없으면 null)", "geuk": "극 있을 때 1문장 (없으면 null)" },
  "conflictScenarios": [
  ...
  "emotionalPattern": { "myPattern": "내 감정 반응 1문장", "targetPattern": "상대 반응 1문장", "cycle": "반복 사이클 1-2문장" },
  "energyDynamic": { "whoLoses": "누가 더 소모되는지 1문장", "drainMechanism": "소모 방식 1문장", "longTermEffect": "장기 전망 1문장" },
  ...
  "avoidanceGuide": { "mindset": "마음가짐 1-2문장", "practicalTips": ["팁1 (구체적 행동)", "팁2", "팁3"], "boundaries": "절대 기대하면 안 되는 것 1문장" }
}`;
  }

  return `이 사주의 "내 위험 유형" 분석 (01~03 섹션). 단정적으로.
...

{
  ...
  "myCharacter": { "core": "핵심 기질 2문장", "strength": "관계에서 강점 1문장", "shadow": "관계에서 그림자 1문장" },
  "dangerTypes": [
  ...
  "avoidanceGuide": { "mindset": "마음가짐 1-2문장", "practicalTips": ["팁1", "팁2", "팁3"], "boundaries": "타협하면 안 되는 것 1문장" }
}`;
}

function buildPhase2Prompt(myData: any, targetData: any, relationType: string, result: any): string {
  return `두 사람의 사주로 04·05 섹션만 분석해주세요.
...
[충돌] 충: ${result.conflicts.chung.map((c: any) => c.name).join(', ') || '없음'} / 오행극: ${result.conflicts.geuk.direction || '없음'}

순수 JSON으로만 반환:

{
  "personalImpact": { "onMe": "이 관계가 나에게 하는 일 1-2문장", "warningSignals": ["신호1 (구체적 증상)", "신호2", "신호3"], "whatYouLose": "잃어가는 것 1문장" },
  "continuationAssessment": { "structuralAnalysis": "구조적으로 개선 가능한지 1-2문장", "whatItTakes": "이어가려면 필요한 것 1문장", "redLine": "재고해야 할 신호 1문장", "verdict": "최종 판정 1문장. 가장 직접적으로" }
}`;
}
...
export default async function handler(req: any, res: any) {
  ...
  const maxTokens = phase === 1 ? 3000 : 1000;
